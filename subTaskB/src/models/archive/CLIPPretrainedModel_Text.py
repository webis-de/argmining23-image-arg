...
def check_character_count(image_path: str) -> torch.Tensor:
...
def train_and_evaluate(
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(len(train_dataset)):
                ...
                image_path = train_dataset[i]["image_path"]
                ...
                with torch.no_grad():
                    ...
                    character_count_feature = check_character_count(image_path)
                ...
                logits = custom_model(joint_embedding,
                                      character_count_feature.to(device))
...
def evaluate(
        ...
        with torch.no_grad():
            for i in range(0, len(valid_dataset)):
                ...
                image = preprocess(image).unsqueeze(0).to(device)
                character_count_feature = check_character_count(image)
...
if __name__ == "__main__":
        ...
        character_count_feature_size = 1

        custom_model = CustomModel(num_classes, clip_feature_size, character_count_feature_size)
        custom_model = custom_model.to(device)
        downstream_model = custom_model
